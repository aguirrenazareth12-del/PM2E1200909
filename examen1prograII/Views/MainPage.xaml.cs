...
public partial class MainPage : ContentPage
{
    ...
    private async void TomarFoto(object sender, EventArgs e)
    {
        try
        {
            ...
            using FileStream destino =
                File.OpenWrite(rutaFoto);

            await origen.CopyToAsync(destino);

            imgSitio.Source =
                ImageSource.FromFile(rutaFoto);
        }
        catch (Exception ex)
        {
            await DisplayAlert(
    ...
    private async void GuardarSitio(
        object sender,
        EventArgs e)
    {
